...
The [NVIDIA TAO Toolkit](https://docs.nvidia.com/tao/tao-toolkit/index.html) is a 
low-code version of the NVIDIA TAO framework that 
accelerates the model training process. The TAO Toolkit lets you use 
transfer learning to fine-tune NVIDIA pretrained models with your own data and to optimize for 
inference—-without AI expertise or large training datasets.

Integrate ClearML when working with TAO Toolkit. ClearML automatically logs outputs generated by TAO, including metrics, 
model files, plots, and debug samples, so you can gain more insight into the training process.  
...
1. Connect the ClearML SDK to the server by creating credentials (go to the top right in the UI to **Settings > Workspace > Create new credentials**), 
   ...
   You can set the environment variables with your ClearML credentials via the TAO Toolkit launcher. Add your credentials 
